...
namespace ots::store {
    ...
    private:
        int ResetKey(store_data &key) {
            ...
            // change key id
            store_header_->key_id = store_header_->key_id == 0 ? 1 : 0;
            if (store_header_->key_id == 0) {
        ...
        int ResetValue(store_data &value) {
            ...
            // change value id
            store_header_->value_id = store_header_->value_id == 0 ? 1 : 0;
            if (store_header_->value_id == 0) {
    ...
    public:
        ...
        int Init(const std::string &file_path,
            ...
            // 约束最大内存空间，在不同系统泛用
            {
                size_t header_size = sizeof(store_header);
                size_t item_size = sizeof(store_item);
                size_t k_size = KeyMaxSize;
        ...
        int Get(store_data &key, store_data &value) {
            const uint8_t reader_id = store_header_->reader_id;
            if (reader_id == 0) {
        ...
        int Set(store_data &key, store_data &value) {
            ...
            *writer_item_used_ = *reader_item_used_;
            ...
            SPDLOG_DEBUG("item_id:{}, writer_item_id:{}, writer_item_used:{}", store_header_->item_id, writer_item_id,
                         *writer_item_used_);
            ...
            if (ret == 0) {
                // change reader-item
                store_header_->item_id = writer_item_id;
                if (store_header_->item_id == 0 && store_header_->key_id == 0 && store_header_->value_id == 0) {
                    store_header_->reader_id = 0;
                } else if (store_header_->item_id == 0 && store_header_->key_id == 0 && store_header_->value_id == 1) {
                    store_header_->reader_id = 1;
                } else if (store_header_->item_id == 0 && store_header_->key_id == 1 && store_header_->value_id == 0) {
                    store_header_->reader_id = 2;
                } else if (store_header_->item_id == 0 && store_header_->key_id == 1 && store_header_->value_id == 1) {
                    store_header_->reader_id = 3;
                } else if (store_header_->item_id == 1 && store_header_->key_id == 0 && store_header_->value_id == 0) {
                    store_header_->reader_id = 4;
                } else if (store_header_->item_id == 1 && store_header_->key_id == 0 && store_header_->value_id == 1) {
                    store_header_->reader_id = 5;
                } else if (store_header_->item_id == 1 && store_header_->key_id == 1 && store_header_->value_id == 0) {
                    store_header_->reader_id = 6;
                } else {
                    store_header_->reader_id = 7;
                }
            }
            return ret;
        }
    ...
    public:
        ...
        void ShowHeader() {
            SPDLOG_INFO("store_header-item_id:{}", store_header_->item_id);
            SPDLOG_INFO("store_header-key_id:{}", store_header_->key_id);
            SPDLOG_INFO("store_header-value_id:{}", store_header_->value_id);
        ...
        void ShowCurrentKey() {
            if (store_header_->item_id == 0) {
                ...
                for (int i = 0; i < store_header_->item_a_used; i++) {
                    ...
                    auto *key_buffer =
                            store_header_->key_id == 0 ? store_data_address_.key_a_address : store_data_address_.key_b_address;
                    char tmp[128] = {0};
        ...
        void ShowAllKey() {
            SPDLOG_INFO("item_id:{}, key_id:{}, value_id:{}", store_header_->item_id, store_header_->key_id, store_header_->value_id);
            SPDLOG_INFO("item-a");
            for (int i = 0; i < store_header_->item_a_used; i++) {
                auto *item = (store_item *) store_data_address_.item_a_address + i;
                auto *key_buffer = store_header_->key_id == 0 ? store_data_address_.key_a_address : store_data_address_.key_b_address;
                char tmp[128] = {0};
                memcpy(&tmp, key_buffer + item->key_offset, item->key_len);
                SPDLOG_INFO("item:{}, key:{}, key_offset:{}, key_len:{}, value_offset:{}, value_len:{}",
                            i, tmp, item->key_offset, item->key_len, item->value_offset, item->value_len);
